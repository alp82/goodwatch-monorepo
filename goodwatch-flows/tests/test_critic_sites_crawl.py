...
class CrawlTestCase(unittest.TestCase):
    ...
    def add_show(self, tmdb_id, title, year, popularity=50.0, url=None, wikidata_url=None, source=None,
                 airing=False, imdb_id=None, next_crawl_at=crawl.EPOCH, **rating):
        details = {"tmdb_id": tmdb_id, "title": title, "original_title": title, "popularity": popularity,
                   "first_air_date": datetime(year, 1, 20), "in_production": airing,
                   "last_air_date": datetime(2026, 9, 1) if airing else datetime(year + 3, 1, 1),
...
class MetacriticTests(CrawlTestCase):
    ...
    def test_a_different_imdb_id_rejects_even_a_wikidata_url(self):
