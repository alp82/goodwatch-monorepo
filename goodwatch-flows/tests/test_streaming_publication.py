...
def availability(country="US", **fields):
...
class StreamingPublicationTests(unittest.TestCase):
    ...
    def test_all_pending_retains_existing_availability_without_freshness(self):
    ...
    def test_mixed_success_replaces_only_verified_country_and_rebuilds_aggregate(self):
    ...
    def test_processed_empty_clears_scraped_fields_but_retains_api_and_other_countries(self):
    ...
    def test_processed_empty_removes_obsolete_scrape_offer(self):
        self.db.tmdb_tv_providers.insert_one({"tmdb_id": 42, "country_code": "US", "updated_at": self.now, "streaming_links": []})
        crate = Crate([availability()])
        self.publish(crate)
    ...
    def test_absent_providers_publish_verified_api_without_inventing_scrape_freshness(self):
    ...
    def test_absent_sources_are_explicit_and_do_not_write(self):
        crate = Crate([availability()])
        result = self.publish(crate)
        self.assertFalse(any(sql != "REFRESH TABLE streaming_availability" for sql in crate.writes))
        self.assertEqual(result["publication"]["titles"]["42"]["provider_state"], "absent")

    def test_api_empty_clears_only_api_contribution_while_scrape_is_pending(self):
        self.db.tmdb_tv_details.update_one({"tmdb_id": 42}, {"$set": {"updated_at": self.now,
    ...
    def test_failed_country_retains_published_data_despite_previous_success(self):
    ...
    def test_scheduled_catchup_uses_same_country_retention(self):
        self.db.tmdb_tv_providers.insert_many([
    ...
    def test_concurrent_publication_fails_before_mutating_children_or_aggregate(self):
        self.db.tmdb_tv_providers.insert_one({"tmdb_id": 42, "country_code": "US", "updated_at": self.now, "streaming_links": []})
    ...
    def test_replaced_lease_prevents_writes_and_preserves_new_owner(self):
        self.db.tmdb_tv_providers.insert_one({"tmdb_id": 42, "country_code": "US", "updated_at": self.now, "streaming_links": []})
        crate = Crate([availability()])
        select = crate.select
    ...
    def test_failed_child_write_does_not_publish_aggregate_and_releases_own_lease(self):
        self.db.tmdb_tv_providers.insert_one({"tmdb_id": 42, "country_code": "US", "updated_at": self.now,
            "streaming_links": [{"provider_name": "Amazon", "stream_type": "flatrate", "stream_url": "https://new"}]})
        crate = Crate([availability()])
        def fail(**kwargs):
