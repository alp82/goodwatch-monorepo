...
def main(next_ids: dict) -> list[dict]:
    ...
    if not isinstance(next_ids, dict) or not all(
        key in next_ids for key in ("movie_ids", "tv_ids")
    ):
        raise ValueError(
            "Expected movie_ids and tv_ids containing provider document IDs"
        )
    validated = {}
    ...
    try:
        ...
        for media_type, ids in validated.items():
            for identity in ids:
                document = db[f"tmdb_{media_type}_providers"].find_one(
                    {"_id": ObjectId(identity)}, {"tmdb_id": 1}
                )
                if document is None:
                    raise ValueError(
                        f"Missing {media_type} provider document {identity}"
                    )
                entries.append(
                    {
                        "id": identity,
