...
def eligibility(now: datetime | None = None) -> dict:
    ...
    return {"$and": [
        {"$or": [{"next_fetch_at": {"$lte": now}}, {"next_fetch_at": None, "$or": [
            {"updated_at": None}, {"updated_at": {"$lte": now - FRESHNESS}},
        ]}]},
        {"$or": [{"lease_expires_at": None}, {"lease_expires_at": {"$lte": now}}]},
    ]}


def claim(db: Database, collection: Collection, identity: ObjectId, now: datetime | None = None) -> dict | None:
    now = now or datetime.utcnow()
    if upstream_deadline(db, now) is not None:
        return None
    return collection.find_one_and_update(
        {"_id": identity, **eligibility(now)},
        {"$set": {"lease_token": uuid4().hex, "lease_expires_at": now + LEASE_DURATION}},
        return_document=ReturnDocument.AFTER,
    )


def fence(document: dict, now: datetime) -> dict:
    return {"_id": document["_id"], "lease_token": document["lease_token"], "lease_expires_at": {"$gt": now}}


def save_success(collection: Collection, document: dict, links: list[dict], now: datetime | None = None) -> bool:
    now = now or datetime.utcnow()
    result = collection.update_one(fence(document, now), {
        "$set": {"streaming_links": links, "updated_at": now,
                 "next_fetch_at": now + FRESHNESS, "consecutive_failures": 0,
                 "is_selected": False},
        "$unset": {"lease_token": "", "lease_expires_at": "", "error_message": "", "failed_at": ""},
    })
    return result.modified_count == 1


def upstream_deadline(db: Database, now: datetime | None = None) -> datetime | None:
    now = now or datetime.utcnow()
...
def save_failure(db: Database, collection: Collection, document: dict, error: object, *,
                 rate_limited: bool = False, retry_at: datetime | None = None,
    ...
    result = collection.update_one(fence(document, now), {
        "$set": {"failed_at": now, "error_message": str(error)[:1000],
                 "next_fetch_at": deadline, "consecutive_failures": failures,
                 "is_selected": False},
        "$unset": {"lease_token": "", "lease_expires_at": ""},
    })
    if rate_limited:
        # Even a worker whose lease expired observed a real upstream limit.
        db.tmdb_streaming_upstream.update_one({"_id": "tmdb_watch"},
            {"$max": {"blocked_until": deadline}}, upsert=True)
    return result.modified_count == 1
...
def country_from_url(url: str | None, tmdb_id: int, media_type: str) -> str:
    ...
    if not isinstance(url, str) or not isinstance(tmdb_id, int) or isinstance(tmdb_id, bool) or tmdb_id <= 0:
    ...
    if (parsed.scheme != "https" or parsed.hostname not in {"www.themoviedb.org", "themoviedb.org"}
            ...
            or not re.fullmatch(rf"/{expected}/{int(tmdb_id)}(?:-[^/]*)?/watch/?", parsed.path)
            or len(locales) != 1 or not re.fullmatch(r"[A-Za-z]{2}", locales[0])):
...
def identity_map(documents: list[dict], media_type: str) -> tuple[dict, dict]:
    ...
    for document in documents:
        try:
            country = country_from_url(document.get("tmdb_watch_url"), document["tmdb_id"], media_type)
    ...
    for country, matches in countries.items():
        if len(matches) > 1:
            for document in matches:
                errors[document["_id"]] = f"Duplicate identity: {document['tmdb_id']}/{country}"
...
def normalization_update(document: dict, country: str) -> tuple[dict, dict]:
    ...
    if document.get("next_fetch_at") is None:
        fields["next_fetch_at"] = (document["updated_at"] + FRESHNESS
                                   if document.get("updated_at") else datetime(1970, 1, 1))
    if document.get("consecutive_failures") is None:
        fields["consecutive_failures"] = 0
    selector = {"_id": document["_id"], **{key: document.get(key) for key in (
        "updated_at", "next_fetch_at", "lease_token", "tmdb_watch_url", "country_code")}}
    return selector, {"$set": fields}



def normalize_document(collection: Collection, document: dict, country: str, now: datetime) -> bool:
...
def ensure_indexes(collection: Collection) -> None:
    collection.create_index([("next_fetch_at", 1), ("lease_expires_at", 1)], name="country_eligibility")
    collection.create_index([("tmdb_id", 1), ("country_code", 1)], unique=True,
        partialFilterExpression={"country_identity_ready": True}, name="validated_country_identity")


def backfill(collection: Collection, media_type: str, after_id: ObjectId | None = None,
             batch_size: int = 1000, dry_run: bool = False, now: datetime | None = None) -> dict:
    now = now or datetime.utcnow()
    ...
    documents = list(collection.find(selector).sort("_id", 1).limit(batch_size))
    identities = {}
    errors, updated = [], 0
    for document in documents:
        tmdb_id = document.get("tmdb_id")
        if tmdb_id not in identities:
            identities[tmdb_id] = identity_map(list(collection.find({"tmdb_id": tmdb_id})), media_type)
        countries, invalid = identities[tmdb_id]
        if document["_id"] in invalid:
            errors.append({"id": str(document["_id"]), "error": invalid[document["_id"]]})
            continue
        country = next(country for country, matches in countries.items() if any(d["_id"] == document["_id"] for d in matches))
        if not dry_run:
            updated += normalize_document(collection, document, country, now)
    return {"scanned": len(documents), "updated": updated, "errors": errors,
            "after_id": str(documents[-1]["_id"]) if documents else None,
            "done": len(documents) < batch_size, "dry_run": dry_run}


def initialize_countries(collection: Collection, tmdb_id: int, providers: dict, media_type: str,
                         original_title: str | None = None, popularity: float | None = None) -> dict:
    ...
    now = datetime.utcnow()
    for country, matches in countries.items():
        normalize_document(collection, matches[0], country, now)
    for country_key, provider in providers.items():
        ...
        try:
            inserted = collection.update_one(selector, {"$setOnInsert": {
                "tmdb_watch_url": url, "created_at": now,
                "next_fetch_at": datetime(1970, 1, 1), "consecutive_failures": 0,
                "original_title": original_title, "popularity": popularity,
            }}, upsert=True)
