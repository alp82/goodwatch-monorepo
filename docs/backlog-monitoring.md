...
Priority aggregation counts demand above acknowledgment only after the seven-day success cooldown, any active priority lease, and a **two-hour grace period**. The age is a conservative lower bound from the last queue update: claims and additional demand can advance that timestamp, so it is not represented as the original demand creation time. A grouped publication incident opens after a further **two hours without acknowledgment/backlog progress**.
