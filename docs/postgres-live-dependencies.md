...
## Windmill application credential removal

At 2026-09-09 16:21 UTC, after backend deployment and legacy-consumer retirement, the backed-up application PostgreSQL resource `u/Alp/resource/postgresql` and the five variables `u/Alp/POSTGRES_DB`, `POSTGRES_HOST`, `POSTGRES_PASS`, `POSTGRES_PORT`, and `POSTGRES_USER` were removed. Their current definitions matched the protected originals before deletion; subsequent GET requests returned 404. The associated secret variable at `u/Alp/resource/postgresql` also became absent when the resource was removed, so no separate delete was needed for it.

The two ignored local PostgreSQL resource/variable YAML files were byte-compared with their backups and removed. Original definitions remain in the protected `postgres-credentials-20260909T161445Z` backup; the exact operation journal is `postgres-credential-removal.json`. CrateDB credentials, Windmill’s own database configuration, Coolify environment variables, and database services were not changed by this step.
