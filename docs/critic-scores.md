...
| IMDb owner | A Metacritic page whose IMDb id no title in the group has joins the group with the one catalog title of that kind whose TMDB IMDb id it is (its rating document), unless that title is already verified by IMDb id on another page. It usually wins, and a title it beats keeps its own reason (`imdb_mismatch`). A title that loses a page, to its owner or as a `duplicate`, then tries its next URL. Wikidata swapped the Heartland pages: CBC's (14929) has TNT's `tv/heartland`, and TNT's (2756) held CBC's `heartland-2007`. |
