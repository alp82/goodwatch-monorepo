...
2026-09-23. Tuned on dev+ = splits `dev` and `holdout` (the holdout split is dev since the holdout verdict; contract
Notes). `holdout2` was not read, run or scored. Lists: `harness/run4.py final` (configs `run4.FINAL`), tables:
`harness/run4.py report`. Every top 10 below is fully graded (`unj10` 0): 122 new pairs in two passes (118 from a
broad pool of sweep candidates, A/B quadratic weighted kappa 0.866, exact 75.4%; 4 from the frozen finalists, kappa
1.0); no pair was 2+ apart, so assessor C wasn't needed. Merged add-only into `results/grades.json` (2,553 → 2,675).
Files: `results/grading/r4-p1-*`, `r4-p2-*`. The new grades raise some ideal DCGs, so round-3 and holdout numbers
moved (prod holdout 0.461 → 0.450, r3-combo 0.756 → 0.740); compare within these tables only.

## Configs

| ranker | = | round-4 changes |
|---|---|---|
| r4-combo | r3-combo + | reference titles, body-only sparse, facet coverage 0.3, sparse bigram weight 2, title-kind needs every concrete word |
| r4-combo-fast | r3-combo-fast + | the same (no Crate; every added signal is one more list in the same Qdrant batch) |
| r4-fast-noref / -nocov / -bigram1 | r4-combo-fast − | one change removed each (ablations) |

`_R4` in run4.py: `ref=True, ref_mix=0.2, ref_text="full", ref_agree=0.2, ref_sparse="body", sparse_body=True, cov=0.3,
cov_k=300, cov_mode="z", cov_beta=0.5, cov_fields="body", sparse_bigram=2.0`; blend `kind_all=True`.

## Summary

Contract rule: no query more than 0.15 below prod. Both finalists: none (closest: lab-04 "tense heist thriller, not
bleak" −0.104 / −0.144, core-18 −0.077 / −0.082, new-01 −0.075 / −0.083).
