...
WCRED = dict(SAFE, **V2, credits_file="credits-v2.jsonl.gz")   # combo-safe-v2-wcred

FINAL = {
    # negated concrete nouns ("space opera without aliens"): titles whose keywords / tags hold the negated words drop
    "neg-base": variant(**WCRED),   # the same as simp_combo's combo-safe-v2-wcred, for the new dev6 queries
    "neg-lex05": variant(**dict(WCRED, neg_lex=0.5)),
    "neg-lex1": variant(**dict(WCRED, neg_lex=1.0)),
    "neg-lex2": variant(**dict(WCRED, neg_lex=2.0)),
}
