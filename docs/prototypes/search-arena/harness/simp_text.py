"""The flat search ranking (simp.py) with simpler general text rules: `rank(ctx) -> list`.
    ...
    if cfg["facet"]:
        if ref and mod and cfg["ref_facet"]:
            ...
            if ref:
                ...
                fcs = fcs if len(fcs) >= cfg["facet_min_n"] else []
            for f in fcs:
...
def rank_filmography(ctx, det, cfg, non_en):
    ...
    q_text, negated = entity_query(ctx, det, cfg, non_en)
    mask = era_filter(ctx, cfg, ctx.mask)
    rows = np.flatnonzero(mask)
    ...
    if negated and cfg["neg"]:
        ...
        score = score - cfg["neg"] * z(np.max(np.stack([x[cand] for x in pens]), axis=0))
    p, q = cfg["prior"]
    if p or q:
        lv, gw = prior_terms(cand)
        score = score + (p * z(lv) + q * z(gw))
...
def rank_style(ctx, det, cfg, non_en):
    ...
    if negated and cfg["neg"]:
        pens = [Em @ qemb.embed(emb_main, n) for n in negated]
        terms.append(-cfg["neg"] * z(np.max(np.stack([x[cand] for x in pens]), axis=0)))
    lv, gw = prior_terms(cand)
