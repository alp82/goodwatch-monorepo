...
const readFlags = async (request: string, criteria: "full" | "short" = "full", routing = false) => {
	const usage = newUsage()
	const questions: Record<string, unknown> = {}
	...
	if (routing) questions.search_route = {
        type: "choice",
        instructions: "Using only the wording of `request`, which search action is intended? You have no catalog or title knowledge. A short name or fragment can be ambiguous. A comparison to a named work with additional preferences is a description request.",
        criteria: {
            lookup: "Find a particular movie, show, or person by name",
            description: "Discover something matching described content, mood, or preferences",
            uncertain: "The wording alone does not distinguish a name from a description or unfinished input",
        },
    }
	...
	return { request, flags, genres, usage, shape, split, phrases, lean: LEAN_ATTRIBUTES, routing: answers.search_route as ChoiceAnswer | undefined }
}
					...
					return (
						await query<TextRow>(
							`SELECT ${columns} FROM ${table}
							 WHERE goodwatch_overall_score_voting_count >= ${MIN_VOTES} AND essence_text IS NOT NULL
...
// THROWAWAY translation failure experiment; accepted D4+ entry point above is unchanged.
export const runNativeVectorTranslationFallback = async (request: string) => {
    const started = Date.now()
    const [attributes, reading] = await Promise.all([readFlags(request, "full"), readWantAvoid(request, "short")])
    const weights = pick(reading.details, d => d.weight >= 0.6 || d.weight <= -1.2, 74)
    const result = await retrieve({weights, vector: sparseVector(weights), order: "weighted_sum", genres: []}, attributes.flags, "")
    return {results: result.results, notes: ["Translation unavailable: native Jev reading, vector retrieval only; no native text evidence."],
        ms: Date.now() - started, tokens: attributes.usage.tokens + reading.usage.tokens,
        usd: attributes.usage.usd + reading.usage.usd, model: MODEL, textEvidence: false}
}
