...
The original combined prototype retains its 20-result default. The shared D4+ retrieval accepts an optional bounded result limit; the connected search requests 100. Refinements apply to returned results, not a new full-catalog query. Metadata comes from a bounded read-only Crate lookup. Adult-flagged rows are hidden when known; unknown classification remains eligible. Reliable matching identity is collapsed without merging by title alone.

Removing the discovery vote floor, adult opt-in across retrieval sources, and editable inferred intent still need real retrieval contracts. The old simulated controls were removed. Do not interpret a filtered empty returned list as proof that the whole catalog lacks matches. Search result people are shown as known-for summaries, not linked to movie/show detail routes.
...
The search status now shows a cyan loading band and spinner from query editing/debounce through title, Jev, and metadata completion. Previous results remain clickable; filters and the header stay enabled. The live status is outside the busy result list so assistive technology can announce it. The redundant Focus header search button is removed.

The connected search retrieves up to five title-catalog pages and up to 100 description candidates, blends to at most 100 results, and shows 20 per page. Metadata requests are split into validated batches of at most 40. This is bounded pagination over one ranked search snapshot, not an exhaustive catalog total. Page changes reuse the snapshot without another Jev request. Query/filter edits reset pagination; page navigation pushes history; detail links retain the corresponding page, including Next/Previous across page boundaries. Keyboard selection follows the visible page. The session cache version changed so old 20-result snapshots are not reused.

Browser QA: Heat returned 100 results across five pages; pages 1 and 2 had no overlap; page 2 fetched no new search data; opening details and Back returned to page 2. During an intentionally delayed Drive request, loading was visible after 100ms (debounce) and after 1.3s (fetch), all 20 previous rows remained visible, and changing the Titles filter succeeded. Completion showed the new results. No focus button, no mobile overflow, and no page errors. Accessibility remains 95 with existing footer/shell findings, and TypeScript remains at 269 existing diagnostics with none in the changed search modules.
