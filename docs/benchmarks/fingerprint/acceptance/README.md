...
**Subsequent update:** [The Matrix and Preacher have been regenerated and verified](regeneration.md) after the highlight-validator fix. The report below records the original acceptance run; its statements about invalid stored records describe that earlier state. [The actual monthly guardrail response has also been verified](monthly-live.md); the test limit/pause cleanup is complete, and cost reconciliation remains open.
