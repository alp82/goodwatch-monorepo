import InterestDiscovery from "~/ui/discovery/InterestDiscovery"
...
function getAccuracyLabel(ratingsCount: number): {
	label: string
	color: string
} {
	if (ratingsCount < 10)
		return {
			label: "Accuracy improving with each rating",
...
export default function RecommendationsView({
	...
	const currentRecommendations =
		variant === "fingerprint" && selectedKey && fingerprintData
			? fingerprintData.recommendations[selectedKey] || []
			: []

	return (
		<div className="relative w-full flex flex-col bg-gray-900/95 rounded-2xl border border-gray-700/50 backdrop-blur-sm">
			...
			<div className="flex flex-col px-4 md:px-6 py-2 md:py-4 border-b border-gray-700/50 bg-gray-800/50">
				<div>
					<h2 className="text-lg md:text-2xl font-semibold text-white">
						{currentFeature?.icon}{" "}
						{currentFeature?.name || "Explore movies and shows"}
					</h2>
					<div className="text-sm md:text-lg text-gray-400">
						{ratingsCount > 0
							? `Your progress: ${ratingsCount} ratings`
							: "No ratings required. Refine your taste whenever you like."}
					</div>
				</div>
			</div>
			...
			<div className="flex-1 px-4 md:px-6 py-4 overflow-hidden">
				{variant === "fingerprint" && fingerprintData ? (
					<div className="h-full flex flex-col gap-2">
						<InterestDiscovery />
						<h3 className="mt-6 font-semibold">More from your Fingerprint</h3>
						{/* Fingerprint Tabs */}
						<FingerprintTabs
								...
								<div className="my-1 md:my-4">
									<h4 className="flex flex-col md:flex-row md:gap-2 text-md">
										...
										<span className="text-white font-bold">
											{getFingerprintMeta(selectedKey).description}
										</span>
									</h4>
								</div>
			...
			{/* Footer Actions */}
			{isGuest && (
				<div className="flex flex-col px-4 md:px-6 py-2 md:py-4 border-t border-gray-700/50 bg-gray-800/50">
