...
export default function FilterKeywords({
	...
	const keywordsToExclude: Keyword[] =
		withoutKeywordsFetcher.data?.keywords || [];

	const autocompleteFetcher = useFetcher();
	const autocompleteItems = (
		autocompleteFetcher.data?.keywords?.results || []
	).map((keyword: Keyword) => {
		return {
			key: keyword.id,
			label: keyword.name,
	...
	const handleSelect = (selectedItem: AutocompleteItem) => {
		const updatedKeywordsToInclude = [
			...keywordsToInclude,
			{
	...
	const handleDelete = (keywordToDelete: Keyword) => {
		const updatedKeywordsToInclude = keywordsToInclude.filter(
			(keyword) => keyword.id !== keywordToDelete.id,
		);
		onChange(updatedKeywordsToInclude, keywordsToExclude);
	};
