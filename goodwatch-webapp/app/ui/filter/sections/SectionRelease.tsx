...
interface SectionReleaseParams {
	params: Partial<Pick<DiscoverParams, "minYear" | "maxYear">>
	onChange?: (
		params: Partial<Pick<DiscoverParams, "minYear" | "maxYear">>,
	) => void
	initializeOnEdit?: boolean
	editing: boolean
	onEdit: () => void
	onClose: () => void
}
...
export default function SectionRelease({
	...
	useEffect(() => {
		const defaultPreset = initializeOnEdit
			? presets[0]
		...
		if (
			...
			setYearValues([
				params.minYear
					? Number.parseInt(params.minYear)
					: defaultPreset.minYear,
				params.maxYear
					? Number.parseInt(params.maxYear)
					: defaultPreset.maxYear,
			])
			return
		}
