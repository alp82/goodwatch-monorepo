// PROTOTYPE - throwaway. "Rental card": a video-store checkout card half out of its kraft pocket.
...
function CheckoutCard({ title, items, name, theme, date, editing }: CardProps) {
				...
				<div style={col({ position: "relative", width: CARD_W, height: CARD_H, backgroundColor: PAPER, borderRadius: 10, boxShadow: `0 30px 60px ${alpha("#000000", 0.55)}`, padding: "40px 48px 0 48px" })}>
					...
					<div style={col({ position: "relative", height: rowH * lines })}>
						...
						{Array.from({ length: lines }, (_, i) => {
							const item: ListItem | undefined = items[i]
...
export const checkout: Design = { key: "checkout", name: "Rental card", format: "Post 4:5", w: W, h: H, max: 5, Card: CheckoutCard }
