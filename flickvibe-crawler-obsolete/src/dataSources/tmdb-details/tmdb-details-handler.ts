...
import fs from 'fs'
import { FetchedMovieData, FetchedTvData } from './dataSourceTMDBDetails'
...
export const saveTMDBMovies = async (dataList: FetchedMovieData[]): Promise<BulkUpsertResult | undefined> => {
  const movieDetails = dataList.map((data) => {
    const { details } = data
    return {
      last_status: 'success',
      last_updated_successfully: new Date(),
  ...
  try {
    const result = await bulkUpsertData(
      'tmdb_movie',
      movieDetails,
      { last_status: 'SOURCE_STATUS', release_year: 'NUMERIC', collection: 'JSONB' },
      ['tmdb_id'],
      ['tmdb_id', 'title', 'release_year'],
    )
    if (result?.all) {
      result.all.forEach((details) => {
...
export const saveTMDBTvs = async (dataList: FetchedTvData[]): Promise<BulkUpsertResult | undefined> => {
  const movieDetails = dataList.map((data) => {
    const { details } = data
    return {
      last_status: 'success',
  ...
  try {
    const result = await bulkUpsertData(
      'tmdb_tv',
      movieDetails,
      { last_status: 'SOURCE_STATUS', release_year: 'NUMERIC' },
      ['tmdb_id'],
      ['tmdb_id', 'title', 'release_year'],
    )
    if (result?.all) {
      result.all.forEach((details) => {
        console.log(`TV: ${details.title} (${details.year})`)
      })
    }
    return result
  } catch (error) {
