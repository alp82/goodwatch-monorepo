      ...
      {keywords?.results && <>
        ...
        <div className="flex flex-wrap gap-2">
          {keywords.results.map((keyword) => (
            <a key={keyword.id} href={`/discover?type=${type}&with_keywords=${keyword.id}`} className="px-2 py-0.5 inline-flex items-center rounded text-xs font-medium border-2 border-sky-600 text-sky-100 bg-sky-800 hover:text-white hover:bg-sky-900">
              {keyword.name}
            </a>
          ))}
        </div>
