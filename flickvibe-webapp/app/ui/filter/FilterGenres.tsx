...
export default function FilterGenres({ type, withGenres = '', withoutGenres = '', onChange }: FilterGenresProps) {
  ...
  const handleDelete = (genreToDelete: Genre) => {
    const updatedGenresToInclude: Genre[] = genresToInclude.filter((genre) => genre.id !== genreToDelete.id)
    onChange(updatedGenresToInclude, genresToExclude)
  }
  ...
  return <div className="flex flex-wrap gap-4">
    ...
    <div className="flex gap-2">
      {genresToInclude.map((genre: Genre) => {
        return <span key={genre.id} className="inline-flex items-center rounded bg-sky-800 px-2 py-0.5 text-xs font-medium text-sky-100">
          <HashtagIcon className="h-4 w-4 text-sky-100" aria-hidden="true" />
          {genre.name}
          <XCircleIcon className="ml-2 h-5 w-5 cursor-pointer text-red-400 hover:text-red-500" onClick={() => handleDelete(genre)} />
        </span>
      })}
    </div>
  </div>
