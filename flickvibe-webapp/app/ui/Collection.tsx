      ...
      {collection && <div className="mb-4">
        ...
        <div className="flex flex-wrap">
          {movies.filter((movie) => movie.tmdb_id !== movieId).map((movie) => {
            return (
              <a key={movie.tmdb_id} className="flex flex-col w-36 border-4 border-transparent hover:bg-indigo-900 hover:border-indigo-900" href={`/movie/${movie.tmdb_id}-${titleToDashed(movie.title)}`}>
                <div>
                  <img
